...
static ULONGLONG file_time(FILETIME value) {
...
void system_metrics_init(SystemMetrics *m) {
    ...
    if (PdhOpenQueryW(NULL, 0, &m->temperature_query) == ERROR_SUCCESS &&
        PdhAddEnglishCounterW(m->temperature_query, L"\\Thermal Zone Information(*)\\Temperature", 0, &m->temperature_counter) ==
            ERROR_SUCCESS)
        m->temperature_available = true;
    else {
        if (m->temperature_query) PdhCloseQuery(m->temperature_query);
        m->temperature_query = NULL;
        m->temperature_counter = NULL;
    }
}
...
static void disable_temperature(SystemMetrics *m) {
    ...
    m->temperature_query = NULL;
    m->temperature_counter = NULL;
    free(m->temperature_buffer);
    m->temperature_buffer = NULL;
    m->temperature_buffer_size = 0;
}

static void read_temperature(SystemMetrics *m, SystemSnapshot *s) {
    if (!m->temperature_available || PdhCollectQueryData(m->temperature_query) != ERROR_SUCCESS) {
        if (m->temperature_available) disable_temperature(m);
        return;
    }
    ...
    if (status == PDH_MORE_DATA) {
        void *buffer = realloc(m->temperature_buffer, size);
        if (!buffer) {
            disable_temperature(m);
            return;
        }
        m->temperature_buffer = buffer;
        m->temperature_buffer_size = size;
    ...
    PDH_FMT_COUNTERVALUE_ITEM_W *items = m->temperature_buffer;
    double hottest = -HUGE_VAL;
    for (DWORD i = 0; i < count; ++i)
        if ((items[i].FmtValue.CStatus == PDH_CSTATUS_VALID_DATA || items[i].FmtValue.CStatus == PDH_CSTATUS_NEW_DATA) &&
            isfinite(items[i].FmtValue.doubleValue) && items[i].FmtValue.doubleValue > hottest)
            hottest = items[i].FmtValue.doubleValue;
    if (isfinite(hottest)) {
        s->temperature = hottest - 273.15;
...
static void read_cpu(SystemMetrics *m, SystemSnapshot *s) {
    ...
    if (m->has_cpu_sample && idle >= m->idle && kernel >= m->kernel && user >= m->user) {
        ...
        if (total && idle_delta <= total) {
            s->cpu = 100.0 * (double)(total - idle_delta) / (double)total;
            s->has_cpu = true;
        }
    }
    m->idle = idle;
...
static void read_memory(SystemSnapshot *s) {
    ...
    s->memory = 100.0 * (double)(memory.ullTotalPhys - memory.ullAvailPhys) / (double)memory.ullTotalPhys;
    s->has_memory = true;
}
...
void system_metrics_dispose(SystemMetrics *m) {
    disable_temperature(m);
}
