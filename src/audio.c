...
static DWORD WINAPI capture_thread(void *parameter) {
    ...
    WAVEFORMATEX *wave = NULL; HANDLE packet_event = NULL; float *converted = NULL;
    ...
    if (!reset_loudness(e)) { ReleaseSRWLockExclusive(&e->lock); set_error(e, L"libebur128 initialization failed."); goto cleanup; }
    ...
    while (running) {
        ...
        for (;;) {
            ...
            if (e->reset_requested) { peak_reset(&e->peak); reset_loudness(e); e->loudness_reset_for_silence = true; e->reset_requested = false; }
            float packet_peak = peak_update(&e->peak, values, samples, (unsigned)channels, now.QuadPart);
            if (e->loudness) ebur128_add_frames_float(e->loudness, values, frames); e->last_packet = now.QuadPart;
...
cleanup:
    AcquireSRWLockExclusive(&e->lock); e->connected = false; if (e->loudness) ebur128_destroy((ebur128_state **)&e->loudness); ReleaseSRWLockExclusive(&e->lock);
    ...
    if (enumerator) IMMDeviceEnumerator_Release(enumerator); if (com) CoUninitialize(); return 0;
}
...
MeterSnapshot audio_snapshot(AudioEngine *e) {
    ...
    if (e->connected && e->loudness) {
        ...
        if (silent) { peak_reset(&e->peak); if (!e->loudness_reset_for_silence) { e->reset_requested = true; e->loudness_reset_for_silence = true; } }
            ...
            if (!ebur128_loudness_momentary(e->loudness, &m) && isfinite(m)) s.momentary = m;
            if (!ebur128_loudness_shortterm(e->loudness, &st) && isfinite(st)) s.short_term = st;
