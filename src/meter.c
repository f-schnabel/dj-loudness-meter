...
static float held_value(PeakMeter *m, int64_t now) {
    ...
    float db = linear_to_db(m->held) - (float)(((double)(now - m->hold_until) / m->frequency) * 18.0);
...
float peak_update(PeakMeter *m, const float *samples, size_t count, unsigned channels, int64_t now) {
    ...
    if (channels == 1) {
        for (size_t i = 0; i < count; ++i) { float v = fabsf(samples[i]); if (v > left) left = v; }
        right = left;
    } else if (channels == 2) {
    ...
    if (m->latest >= held_value(m, now)) { m->held = m->latest; m->hold_until = now + m->hold_duration; }
...
void peak_reset(PeakMeter *m) {
    ...
    memset(m, 0, sizeof(*m)); m->frequency = frequency; m->hold_duration = hold; m->clip_duration = clip;
}

float linear_to_db(float value) { return value > 0.0f ? 20.0f * log10f(value) : -INFINITY; }
...
double display_adjust(double value, double reference) { return isfinite(value) ? value - display_normalize_reference(reference) : value; }
bool display_should_show(double value) { return isfinite(value) && value >= -99.0; }
...
size_t pcm_to_float(const unsigned char *source, size_t bytes, unsigned bits, float *out, size_t capacity) {
    ...
    for (size_t i = 0, p = 0; i < count; ++i, p += width) {
        if (bits == 16) {
            int16_t v = (int16_t)((uint16_t)source[p] | ((uint16_t)source[p + 1] << 8)); out[i] = v / 32768.0f;
        } else if (bits == 24) {
            int32_t v = (int32_t)((uint32_t)source[p] | ((uint32_t)source[p + 1] << 8) | ((uint32_t)source[p + 2] << 16));
            if (v & 0x00800000) v |= (int32_t)0xff000000; out[i] = v / 8388608.0f;
        } else {
            int32_t v; memcpy(&v, source + p, sizeof(v)); out[i] = (float)((double)v / 2147483648.0);
        }
    }
    return count;
}
